...
Usage:
  ...
  python3 moss_tts_profile_no_request_id.py \
    --model-type  local \
...
def get_first_chunk_latency(first_chunk_dir, start_wall_time):
    if not first_chunk_dir or not os.path.exists(first_chunk_dir):
        return None

    first_chunk_ts = []

    for f in os.listdir(first_chunk_dir):
        if not f.endswith(".first_chunk.ts"):
            continue

        ts_path = os.path.join(first_chunk_dir, f)

        try:
            with open(ts_path) as ts_file:
                first_chunk_ts.append(float(ts_file.read().strip()))
    ...
    return max(0.0, min(first_chunk_ts) - start_wall_time)
...
def run_one_sample(omni, ar_params, decoder_params, text, first_chunk_dir, output_dir, sample_idx):
    ...
    # calc FCL 
    # Each sample clears first_chunk_dir before generate(), so any timestamp
    # created there belongs to this run. This avoids relying on internal
    # request_id propagation between stages.
    fcl = get_first_chunk_latency(first_chunk_dir, t0Wall)
...
def profile_mode(mode, sentences, model, repo, output_base, init_sleep, mtype, wandb_name_suffix):
    ...
    run_name = f"vllm-local-inference-profile-{mode}"
    if wandb_name_suffix:
        run_name = f"{run_name}-{wandb_name_suffix}"

    wandbRun = wandb.init(project="hpml-final-project", name=run_name,
    ...
    for i, text in enumerate(sentences):
        ...
        if run["ok"]:
            ...
            wandbRun.log({"sample_idx": i, "mode": mode, "rtf": run["rtf"], "audio_dur_s": run["audio_dur_s"], "first_chunk_latency_s": run["first_chunk_latency_s"], "num_chunks": run["num_chunks"], "total_time_s": run["total_time_s"]})
        else:
...
def main():
    ...
    p.add_argument("--wandb-name-suffix", default="")
    ...
    for mode in args.modes:
        profile_mode(mode=mode, sentences=sentences, model=args.model,  
                     repo=args.repo, output_base=out, init_sleep=args.init_sleep_seconds,
                     mtype=args.model_type, wandb_name_suffix=args.wandb_name_suffix)
